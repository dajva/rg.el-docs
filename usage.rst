...
This version of ``rg.el`` is supported on GNU Emacs
...
met. For the latest version these are:
...
``rg.el`` is using autoloaded symbols which means it's also possible
...
commands. This package is built around recusive search based on three
...
.. command:: rg
   ...
   directory.
   If the the type of the currently visited file is recognized, the
...
the ripgrep binary. ``rg.el`` supports several ways of identifying a
...
An alias is a mapping between a name and a list of `glob patterns <https://en.wikipedia.org/wiki/Glob_(programming)>`_
...
``rg.el`` defines some internal aliases:
...
Pressing the ``rg.el`` :opt:`prefix key <rg-keymap-prefix>` will popup the menu where command
...
triggered but the available options are the same. The visible
options can be controlled by the transient suffix levels
documented `here <https://magit.vc/manual/transient/Enabling-and-Disabling-Suffixes.html#Enabling-and-Disabling-Suffixes>`_.
...
The results buffer is reused when a new search is created. It's
possible to save a search result before invoking a new search with
the following commands.

.. command:: rg-save-search
   :kbd: C-c s s
   
   Save the current search to a buffer with a unique name.

.. command:: rg-save-search-as-name
   :kbd: C-c s S
   
   Save the current search to a buffer and prompt for a name.
...
.. tip:: :cmd:`rg-rerun-change-regexp` and :cmd:`rg-rerun-change-literal` are
   ...
   search string,  just pres the respective key and then ``ENTER``.
...
a function to get a really dynamic setup. See the :opt:`setting's documentation <rg-buffer-name>`
