...
.. code-block:: elisp
    ...
    M-x customize-group [ENTER] rg [ENTER]
...
.. option:: rg-default-alias-fallback
   ...
   recognized for the current buffer. Default setting for this is
   ``all`` and
   ``everything`` is another reasonable value for this variable.
...
.. option:: rg-group-result
   ...
   the ``--no-heading`` ripgrep command line flag. Default value is ``nil``
   (off).
...
.. option:: rg-show-columns
   ...
   Controls if column numbers are used in the search result. Default value
   is ``nil`` (off).
...
.. option:: rg-use-transient-menu
   ...
   Controls wether ``rg-menu`` will be used by default or not. It's also
...
When operating ``rg.el`` in grouped output mode (``rg-group-result`` is non
...
.. code-block:: elisp
    ...
    M-x customize-group [ENTER] rg-face [ENTER]
...
.. function:: (rg-define-search name &rest args)
   ...
   - **:query** - Method for retrieving the search string.  Allowed values are
     ...
     prompt the user for a string.  Any form that evaulates to a string
   ...
   - **:dir** - Root search directory.  Allowed values are ``ask`` for user
     ...
     root.  Any form that evaulates to a directory string is also allowed.
     Default is ``ask``.
